...
CREATE INDEX IF NOT EXISTS idx_entries_created_desc ON entries (created_at DESC)
  WHERE created_at IS NOT NULL;

-- 2. bookmark_countでフィルタ + created_atでソートのパターン用複合インデックス
CREATE INDEX IF NOT EXISTS idx_entries_bookmark_created ON entries (bookmark_count, created_at DESC);

-- 3. 人気エントリー専用の部分インデックス(API minUsers=5, デフォルト値で最も使用頻度が高い)
CREATE INDEX IF NOT EXISTS idx_entries_min5_created ON entries (created_at DESC)
  WHERE bookmark_count >= 5;

-- 4. ホットソート用の部分インデックス(API minUsers=5以上)
CREATE INDEX IF NOT EXISTS idx_entries_hot_partial ON entries (bookmark_count DESC, created_at DESC)
  WHERE bookmark_count >= 5;

-- 5. entry_tagsのカバリングインデックス(loadTags最適化)
CREATE INDEX IF NOT EXISTS idx_entry_tags_entry_covering ON entry_tags (entry_id)
